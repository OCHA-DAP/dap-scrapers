...
def multi(function, s, verbose=False, diff=False, keychange=True, p=True):
    """This version differs in that it attempts to preserve the
    original names in a dictionary"""
    if not keychange:
        index = buildindex(s)
    ...
    for i in s:
        ...
        if showdiff:
            print new[x]
    if p:
        print "%20s %5d %5d %0.4f" % (function.__name__,
                                      len(s),
                                      len(new),
                                      1.0 - (1.0 * len(new)) / len(s)
                                     )
...
def purgewords(i):
    ...
    'buildings'
    """
    purge = ["ltd", "limited", "company", "", "the", "of", "uk", "nhs", "pct",
             "foundation", "trust", "national", "department", "dept",
             "agency", "and", "association", "authority", "co",
             "metropolitan", "british", "consulting", "group", "services",
             "systems"]
    words = i.split(' ')
    for p in purge:
        while p in words:
...
amperstand needs to be before purgewords (ENGL&),
purgechars, anagram. Nice and early.lcase is a requirement for purgewords,
since those words are in lowercase, purgebracket must be
before nopunct, anagram"""

functionlist = [identity, ampersand, lcase, purgebracket, nopunct,
                merge_space, purge_prefix_zero]
shortlist = [identity, ampersand, lcase, nopunct,
             merge_space, purge_prefix_zero]
difflist = []
keepkey = []
